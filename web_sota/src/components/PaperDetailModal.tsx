...
export function PaperDetailModal({
  ...
  useEffect(() => {
    apiGet<{ arxiv_id: string }>(
      `/api/corpus/item?arxiv_id=${encodeURIComponent(paper.paper_id)}`,
    )
      .then(() => setStoreState("stored"))
      .catch(() => {});
  }, [paper.paper_id]);
  ...
  const handleStore = useCallback(async () => {
    ...
    setStoreError(null);
    try {
      await apiPost("/api/depot/ingest", { paper_id: paper.paper_id });
  ...
  const handleCalibre = useCallback(async () => {
    ...
    try {
      await apiPost("/api/calibre/ingest", { paper_id: paper.paper_id });
  ...
  const fetchFullText = useCallback(async () => {
    ...
    try {
      const data = await apiGet<{ markdown: string }>(
        `/api/paper/full-text?paper_id=${encodeURIComponent(paper.paper_id)}`,
      );
  ...
  return (
    <motion.div
      ...
      onClick={onClose}
      data-testid="paper-detail-modal"
    >
      <motion.div
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-border/60">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 text-xs text-muted-foreground font-mono mb-1">
              ...
              {paper.server && (
                <span className="text-primary font-sans">#{paper.server}</span>
              )}
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {paper.categories && paper.categories.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {paper.categories.map((cat) => (
                <span
                  key={cat}
                  className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-primary/5 text-primary/80 border border-primary/10"
                >
                  {cat}
                </span>
              ))}
          ...
          <div className="flex flex-wrap gap-2" data-testid="modal-actions">
            <Button
              size="sm"
              onClick={handleStore}
              disabled={storeState === "storing" || storeState === "stored"}
            >
            ...
            <Button
              ...
              disabled={calibreState === "storing" || calibreState === "stored"}
            >
              <BookMarked className="h-3.5 w-3.5 mr-1.5" />
              {calibreState === "stored"
                ? "Added"
                : calibreState === "storing"
                  ? "Adding..."
                  : "To Calibre"}
            </Button>
            ...
            <a
              href={paper.html_url ?? `https://arxiv.org/abs/${paper.paper_id}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              <Button size="sm" variant="outline">
                <ExternalLink className="h-3.5 w-3.5 mr-1.5" /> Open arXiv
              </Button>
            </a>
          </div>
          ...
          {calibreError && (
            <p className="text-xs text-red-400">{calibreError}</p>
          )}
          ...
          {fullText && (
            <div>
              ...
              <pre className="text-xs leading-relaxed text-foreground/80 whitespace-pre-wrap font-sans bg-muted/30 rounded-lg p-3 max-h-64 overflow-y-auto border border-border/40">
                {fullText}
              </pre>
            </div>
          )}
