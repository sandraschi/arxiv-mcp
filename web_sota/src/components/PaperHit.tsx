...
import { BookMarked, BookOpen, ChevronDown, ExternalLink, FileText, Library } from "lucide-react";
...
function ActionBtn({ state, error, onClick, label, icon: Icon, storedLabel, color }: {
  state: BtnState; error: string | null; onClick: () => void;
  label: string; icon: typeof Library; storedLabel: string; color: string;
}) {
  const isDone = state === "stored";
  return (
    <button onClick={onClick} disabled={state === "storing" || isDone}
      className={cn(
        "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all whitespace-nowrap",
        isDone ? "border-green-600/40 text-green-400 bg-green-950/30 cursor-default" :
        state === "error" ? "border-red-500/40 text-red-400 bg-red-950/30 hover:bg-red-950/50" :
        state === "storing" ? "border-border/40 text-muted-foreground cursor-wait animate-pulse" :
        color === "primary" ? "border-primary/30 text-primary bg-primary/5 hover:bg-primary/10" :
        "border-border/40 text-muted-foreground hover:bg-muted/50 hover:text-foreground"
      )}
      title={error ?? undefined} data-testid={`action-${label.toLowerCase().replace(/\s+/g, "-")}`}
    >
      <Icon className={cn("h-3.5 w-3.5", isDone && "text-green-400")} />
      {state === "storing" ? `${label}...` : state === "stored" ? storedLabel : state === "error" ? "Retry" : label}
    </button>
  );
}
...
export function PaperHit({ p, inDepot, onQuickView }: {
  ...
  const [storeState, setStoreState] = useState<BtnState>(inDepot ? "stored" : "idle");
  ...
  useEffect(() => {
    ...
    apiGet<{ arxiv_id: string }>(`/api/corpus/item?arxiv_id=${encodeURIComponent(p.paper_id)}`)
      .then(() => { setStoreState("stored"); setDepotChecked(true); })
      .catch(() => setDepotChecked(true));
  }, [p.paper_id, inDepot, depotChecked]);

  const handleStore = useCallback(async () => {
    setStoreState("storing"); setStoreError(null);
    try { await apiPost("/api/depot/ingest", { paper_id: p.paper_id }); setStoreState("stored"); }
  ...
  const handleCalibre = useCallback(async () => {
    ...
    try { await apiPost("/api/calibre/ingest", { paper_id: p.paper_id }); setCalibreState("stored"); }
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              {storeState === "stored" && (
                <span className="px-1.5 py-0.5 rounded text-[10px] font-medium border border-green-600/30 text-green-400 bg-green-950/30">
                  In library
                </span>
              )}
            </div>
          ...
          <ChevronDown className={cn("h-5 w-5 text-muted-foreground shrink-0 mt-1 transition-transform duration-200", expanded && "rotate-180")} />
        </div>

        <p className={cn("text-xs text-muted-foreground leading-relaxed mt-2", expanded ? "" : "line-clamp-2")}>
          {p.summary}
        </p>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <div className="p-3 sm:p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-2" data-testid="paper-actions">
                <ActionBtn state={storeState} error={storeError} onClick={handleStore}
                  label="Save to library" icon={Library} storedLabel="Saved" color="primary" />
                <ActionBtn state={calibreState} error={calibreError} onClick={handleCalibre}
                  label="To Calibre" icon={BookMarked} storedLabel="Added" color="default" />
                {onQuickView && (
                ...
                <a href={absUrl} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-border/40 text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-all whitespace-nowrap"
                  onClick={(e) => e.stopPropagation()} data-testid="action-open-arxiv">
                  <ExternalLink className="h-3.5 w-3.5" /> Open arXiv
                </a>
                {p.pdf_url && (
                  <a href={p.pdf_url} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-border/40 text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-all whitespace-nowrap"
                    onClick={(e) => e.stopPropagation()} data-testid="action-pdf">
              ...
              {p.published && (
                <p className="text-[10px] text-muted-foreground">Published: {p.published.slice(0, 10)}</p>
              )}
            </div>
          </motion.div>
        )}
