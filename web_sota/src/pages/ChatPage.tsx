...
import {
  ChevronDown, Copy, Download, Eraser, Pencil, RefreshCw, Send,
} from "lucide-react";
...
const WELCOME_MSG = "Hi! I'm your arXiv research assistant. Ask me to search papers, summarize abstracts, compare approaches, or explore topics in ML, AI safety, neuroscience, and more.";
...
const PERSONALITIES = [
  { id: "research-assistant", label: "Research Assistant", prompt: "You are a helpful research assistant. Be concise and accurate." },
  { id: "expert-reviewer", label: "Expert Reviewer", prompt: "You are an expert peer reviewer. Critically analyze claims, identify weaknesses, and suggest improvements. Be thorough but constructive." },
  { id: "quick-summarizer", label: "Quick Summarizer", prompt: "You specialize in distilling complex topics into 3-bullet summaries: (1) what they did, (2) key result, (3) limitation." },
...
function loadPersonality(): string {
  try { return localStorage.getItem(PERSONALITY_KEY) || "research-assistant"; }
  catch { return "research-assistant"; }
}
      ...
      <div className={cn("group relative max-w-[88%] md:max-w-[78%]", isUser && "order-1")}>
        {editing ? (
          ...
          <>
            ...
            <div className={cn("flex items-center gap-0.5 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity",
...
export function ChatPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      } catch { log("error", "Failed to load skill preprompt"); }
    })();
  }, [log]);
  ...
  const systemPrompt = buildSystemPrompt(skillContent, personalityId, personality.prompt, customPrompt);
  ...
  const send = useCallback(async (overrideMsg?: string) => {
    ...
    try {
      const full = [{ role: "system" as const, content: systemPrompt }, ...next];
  ...
  const clearMessages = useCallback(() => { setMessages([]); localStorage.removeItem(STORAGE_KEY); }, []);

  const exportChat = useCallback(() => {
    if (messages.length === 0) return;
    const blob = new Blob([formatChatTxt(messages)], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `arxiv-chat-${Date.now()}.txt`; a.click();
  ...
  const skillName = skillsList.find((s) => s.name === "arxiv-expert")?.name ?? "arxiv-expert";
        ...
        <div className="flex items-center gap-2">
          ...
          {skillLoaded && (
            <span className="hidden sm:inline-flex items-center gap-1 text-muted-foreground border border-border/40 rounded px-1.5 py-0.5">
              <span className="text-primary">skill:</span>{skillName}
            </span>
          )}
        </div>
      ...
      <Card className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              {skillLoaded && (
                <p className="text-[11px] text-muted-foreground/60">
                  Loaded skill: <span className="text-primary/80 font-mono">{skillName}</span> &mdash; 42 tools available
                </p>
              )}
              <div className="flex flex-wrap justify-center gap-2">
                {["Find papers on mechanistic interpretability", "Summarize recent cs.LG papers", "What's new in AI safety?", "Compare transformer and SSM architectures"].map((s) => (
        ...
        <div className="border-t border-border p-3 md:px-4 flex gap-2 items-end bg-background/80 backdrop-blur-sm">
          <textarea ref={textareaRef} value={input}
            ...
            rows={1} data-edit-mode={editingIdx !== null ? "true" : undefined}
            ...
            placeholder="Ask about papers..." data-testid="chat-input"
            disabled={loading} />
