...
export default function ArxivSearch() {
  ...
  const grouped = useMemo(() => {
    ...
    for (const row of catalog) { const g = row.group || "Other"; if (!m.has(g)) m.set(g, []); m.get(g)!.push(row); }
    return [...m.entries()].sort(([a], [b]) => a.localeCompare(b));
  }, [catalog]);

  const runSearch = useCallback(async () => {
    if (!q.trim()) return;
    setLoading(true); setSearchError(null); setSearched(true);
    try {
      ...
      for (const [srv, info] of Object.entries(data.per_server)) counts[srv] = { label: info.label, count: info.count };
      setPerServer(counts);
      log("info", `Search returned ${data.merged?.length ?? 0} papers across ${Object.keys(data.per_server).length} servers`);
    } catch (e) { setSearchError(String(e)); setPapers([]); setPerServer({}); }
    finally { setLoading(false); }
  }, [q, servers, log]);

  const loadRecent = useCallback(async () => {
    try { const data = await apiGet<{ papers: Paper[] }>(`/api/category/latest?category=${recentCategory}&hours=${recentHours}`); setLatest(data.papers ?? []); }
    catch { /* ignore */ }
  }, [recentCategory, recentHours]);

  const lookupSingle = useCallback(async () => {
    const id = singleId.trim(); if (!id) return;
    setSingleLoading(true); setSingleError(null); setSinglePaper(null);
    try { const data = await apiGet<{ paper: Paper }>(`/api/paper?paper_id=${encodeURIComponent(id)}`); setSinglePaper(data.paper); }
    catch (e) { setSingleError(String(e)); }
  ...
  const searchPresets = [
    ...
    { label: "LLM evaluation", q: "(large language model AND (benchmark OR evaluation OR reasoning))" },
  ];
    ...
    <div className="space-y-6" data-testid="search-page">
      ...
      <Card data-testid="search-card">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowConfig(!showConfig)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
            Options <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", showConfig && "rotate-180")} />
          </button>
        </div>

        <div className="flex flex-wrap gap-2 mt-3">
          {searchPresets.map((p) => (
            <button key={p.label} type="button" onClick={() => setQ(p.q)}
        ...
        <div className="mt-3 flex gap-2">
          <div className="flex-1 relative">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)}
              placeholder='e.g. "consciousness AND transformer"' className="pl-9"
              onKeyDown={(e) => { if (e.key === "Enter") runSearch(); }} />
          </div>
          <Button onClick={runSearch} disabled={loading} data-testid="search-button">
              ...
              <div className="pt-3 space-y-3 border-t border-border/40 mt-3">
                <div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-1">
                    {[["arxiv", "arXiv"], ["biorxiv", "bioRxiv"], ["medrxiv", "medRxiv"], ["chemrxiv", "ChemRxiv"], ["researchsquare", "Research Square"]].map(([key, label]) => {
                      const checked = servers.includes(key);
                      return (
                        <label key={key} className="flex items-center gap-1.5 text-xs cursor-pointer">
                          <input type="checkbox" checked={checked}
                            onChange={() => { const parts = servers.split(",").filter(Boolean); setServers(checked ? parts.filter((s) => s !== key).join(",") : [...parts, key].join(",")); }}
        ...
        {searchError && (
          <div className="mt-4 rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm text-destructive" data-testid="search-error">
            Search failed: {searchError}
          </div>
        )}
        {searched && !searchError && papers.length === 0 && (
          <p className="mt-4 text-sm text-muted-foreground">No results. Try different keywords.</p>
        )}
      </Card>

      <AnimatePresence>
        {papers.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3" data-testid="search-results">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <p className="text-xs text-muted-foreground mt-1">Single arXiv ID lookup.</p>
          <div className="mt-3 flex gap-2">
            <Input value={singleId} onChange={(e) => setSingleId(e.target.value)} placeholder="arXiv ID (e.g. 2401.00001)" className="flex-1"
        ...
        <Card>
          ...
          <p className="text-xs text-muted-foreground mt-1">Recent papers in a subject area.</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <select className={cn(selectClass, "max-w-40")} value={recentCategory} onChange={(e) => setRecentCategory(e.target.value)}>
              {grouped.map(([group, rows]) => (
                <optgroup key={group} label={group}>{rows.map((row) => <option key={row.code} value={row.code}>{row.code}</option>)}</optgroup>
              ))}
            </select>
            <select className={cn(selectClass, "max-w-20")} value={recentHours} onChange={(e) => setRecentHours(e.target.value)}>
              <option value="24">24h</option><option value="72">72h</option><option value="168">7d</option>
            </select>
            <Button onClick={loadRecent} variant="secondary" size="sm">Refresh</Button>
          </div>
          {latest.length > 0 && (
